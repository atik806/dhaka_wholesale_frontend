      ...
      <div className="container py-8">
        <div className="max-w-lg mx-auto">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            <div className="bg-white rounded-[3px] border-2 border-[#E7DCC4] p-6 mb-6 shadow-sm">
              <div className="flex items-center gap-4 mb-6">
                <div className="w-16 h-16 rounded-[2px] bg-[#132A3A] flex items-center justify-center border border-[#E7DCC4]">
                  <User className="w-8 h-8 text-[#F5A300]" />
                </div>
                <div>
                  <p className="font-serif font-bold text-[#132A3A]">
                    {user.name}
                  </p>
              ...
              <form onSubmit={handleSave} className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    value={user.email}
                    disabled
                    className="w-full rounded-[3px] border-2 border-[#E7DCC4] px-4 py-2.5 font-mono text-xs bg-[#FBF6EC] text-[#1C1A17]/50 cursor-not-allowed"
                  />
                </div>
                <div>
                  <label className="block font-mono text-xs font-bold uppercase tracking-wider text-[#132A3A] mb-1.5">
                  ...
                  <input
                    ...
                    required
                    minLength={2}
                    className="w-full rounded-[3px] border-2 border-[#E7DCC4] px-4 py-2.5 font-mono text-xs outline-none focus:border-[#F5A300] focus:ring-2 focus:ring-[#F5A300]/20 bg-white text-[#132A3A]"
                  />
                </div>
                <div>
                  <label className="block font-mono text-xs font-bold uppercase tracking-wider text-[#132A3A] mb-1.5">
                  ...
                  <input
                    ...
                    className="w-full rounded-[3px] border-2 border-[#E7DCC4] px-4 py-2.5 font-mono text-xs outline-none focus:border-[#F5A300] focus:ring-2 focus:ring-[#F5A300]/20 bg-white text-[#132A3A] placeholder:text-[#1C1A17]/40"
                  />
                </div>
                ...
                <button
                  type="submit"
                  disabled={saving}
                  className="flex items-center gap-2 px-5 py-2.5 bg-[#F5A300] hover:bg-[#D88900] text-[#132A3A] rounded-[3px] font-mono text-xs font-extrabold uppercase tracking-wider border border-[#D88900] transition-colors disabled:opacity-50"
                >
                  {saving ? (
            ...
            <div className="bg-white rounded-[3px] border-2 border-[#E7DCC4] mb-6 shadow-sm">
              <button
                onClick={() => setAddrOpen(!addrOpen)}
                className="flex items-center justify-between w-full p-6 text-left hover:bg-[#FBF6EC] transition-colors"
              >
                ...
                >
                  <form onSubmit={handleSaveAddress} className="space-y-4">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <div>
                        <label className="block font-mono text-xs font-bold uppercase tracking-wider text-[#132A3A] mb-1">First Name</label>
                        <input
                          type="text"
                      ...
                      <div>
                        <label className="block font-mono text-xs font-bold uppercase tracking-wider text-[#132A3A] mb-1">Last Name</label>
                        <input
                          type="text"
                          value={addr.lastName || ""}
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <div>
                        <label className="block font-mono text-xs font-bold uppercase tracking-wider text-[#132A3A] mb-1">Email</label>
                        <input
                          type="email"
                          value={addr.email || ""}
                      ...
                      <div>
                        <label className="block font-mono text-xs font-bold uppercase tracking-wider text-[#132A3A] mb-1">Phone</label>
                        <input
                          type="tel"
                          value={addr.phone || ""}
                    ...
                    <div>
                      <label className="block font-mono text-xs font-bold uppercase tracking-wider text-[#132A3A] mb-1">Address</label>
                      <input
                        type="text"
                        value={addr.address || ""}
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <div>
                        <label className="block font-mono text-xs font-bold uppercase tracking-wider text-[#132A3A] mb-1">City</label>
                        <input
                          type="text"
                          value={addr.city || ""}
                      ...
                      <div>
                        <label className="block font-mono text-xs font-bold uppercase tracking-wider text-[#132A3A] mb-1">ZIP Code</label>
                        <input
                          type="text"
                          value={addr.zipCode || ""}
            ...
            <div className="bg-white rounded-[3px] border-2 border-[#E7DCC4] p-6 mb-6 shadow-sm">
              <div className="flex items-center gap-3 mb-4 pb-2 border-b border-[#E7DCC4]">
                <Package className="w-5 h-5 text-[#F5A300]" />
                <h2 className="font-serif font-bold text-[#132A3A]">Order History</h2>
              </div>
              {ordersLoading ? (
                ...
                <p className="font-mono text-xs text-[#1C1A17]/60 text-center py-6">No orders yet</p>
              ) : (
                    ...
                    <div key={order.id} className="py-3 first:pt-0 last:pb-0">
                      <div className="flex items-center justify-between mb-1">
                        <span className={`font-mono text-[10px] font-bold px-2 py-0.5 rounded-[2px] uppercase tracking-wider ${
                          ...
                          order.status === "shipped" ? "bg-[#132A3A]/10 text-[#132A3A] border border-[#132A3A]/30" :
                      ...
                      <div className="flex items-center justify-between">
                        <p className="font-mono text-xs text-[#1C1A17]/60">{formatDate(order.created_at)}</p>
                        <p className="font-mono text-xs text-[#1C1A17]/60 capitalize">{order.payment_method.replace(/_/g, " ")}</p>
                      </div>
                      <div className="mt-1.5 font-mono text-[11px] text-[#1C1A17]/50">
